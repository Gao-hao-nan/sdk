 ...
 * @Description: TODO recyclerView自定义绑定
 */
object RefreshListBind {
    /**
     * AUTHOR:AbnerMing
     * INTRODUCE:列表绑定
     */
            ...
            when (listManager) {
                0 -> {
                    recycler = if (listOrientation == null) {
                        pageRefreshLayout.getRecycler().linear()
                    } else {
                        pageRefreshLayout.getRecycler().linear(RecyclerView.HORIZONTAL)
                    }
                }
                1 -> {
                    recycler = if (listOrientation == null) {
                        pageRefreshLayout.getRecycler().grid(listSpanCount)
                    } else {
                        pageRefreshLayout.getRecycler().grid(listSpanCount, listOrientation)
                    }
                }
                2 -> {
                    recycler = if (listOrientation == null) {
                        pageRefreshLayout.getRecycler().staggered(listSpanCount)
                    } else {
                        pageRefreshLayout.getRecycler().staggered(listSpanCount, listOrientation)
                    }
                }
            }
